...
class Main extends PluginBase implements Listener {
                    ...
                    if(empty($args[0])){
                        ...
                        if(empty($args[1])){
                            ...
                            if($this->getServer()->getPlayer($args[0]) instanceof Player){
                                ...
                                $this->report($sender, $args[0], $args[1]);
                            } else {
    ...
    public function report(Player $player, $reason, $reported){
        foreach($this->getServer()->getOnlinePlayers() as $players){
            if($players->hasPermission("report.sender")){
                $players->sendMessage(TextFormat::GREEN . $reported." was reported by ".$player->getName()." for ". $reason);
            }
        }
    }
}